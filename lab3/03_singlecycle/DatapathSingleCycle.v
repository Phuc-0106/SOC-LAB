...
module DatapathSingleCycle (
  ...
  // ==== ALU control ====
  reg [31:0] alu_op2_in;     // operand B before invert
  reg        alu_op2_invert; // 1: use ~B
  reg        cin_for_cla;    // carry-in for CLA

  // CLA wiring (with invert)
  ...
  always @(*) begin
    ...
    // Mặc định: không ghi reg, không ghi memory
    reg_we        = 1'b0;
    reg_rd_addr   = inst_rd;
    reg_rd_data   = 32'd0;
...
module MemorySingleCycle #(
  ...
  // Đọc lệnh đồng bộ theo clock_mem (như cũ)
  always @(posedge clock_mem) begin
    inst_from_imem <= mem_array[pc_to_imem[AddrMsb:AddrLsb]];
  end

  // ==================== DATA MEMORY (DMEM) ===========================
  // Ghi dữ liệu: đồng bộ, giữ nguyên ở negedge như cũ
  ...
  // Đọc dữ liệu: chuyển sang combinational để Datapath thấy ngay
  always @(*) begin
