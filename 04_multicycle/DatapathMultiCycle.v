...
module DatapathMultiCycle (
  ...
  reg [31:0] latched_rs1_div;
  reg [31:0] latched_rs2_div;

  DividerUnsignedPipelined u_div_pipelined (
    .clk        (clk),
    .rst        (rst),
    .stall      (1'b0),
    .i_dividend (latched_rs1_div), 
    .i_divisor  (latched_rs2_div), 
  ...
  always @(posedge clk) begin
    if (rst) begin
      div_busy        <= 1'b0;
      div_count       <= 4'd0;
      latched_rs1_div <= 32'd0; 
      latched_rs2_div <= 32'd0; 
    end else begin
      if (inst_div_op && !div_busy) begin
          // Bắt đầu chia
          div_busy        <= 1'b1;
          div_count       <= 4'd0;
          latched_rs1_div <= rs1_data; 
          latched_rs2_div <= rs2_data;
          
      end else if (div_busy) begin
        // Đợi đến 15 cycle
        if (div_count == 4'd15) begin
          div_busy  <= 1'b0;
        end else begin
  ...
  reg [31:0] div_q_latched, div_r_latched;
  localparam [3:0] DONE_CYCLE = 4'd15;
  always @(posedge clk) begin
    if (rst) begin
      div_q_latched <= 32'd0;
      div_r_latched <= 32'd0;
    end else if (div_busy && div_count == DONE_CYCLE) begin
      div_q_latched <= div_quotient;
      div_r_latched <= div_remainder;
    end
  end
  ...
  always @(*) begin
    ...
    take           = 1'b0;
     ...
     if ( (inst_div_op && !div_busy) ||               
         (div_busy && (div_count != 4'd15)) ) begin   
    ...
    case (inst_opcode)
      ...
      OpRegReg: begin
        ...
        if (inst_add) begin
          ...
          cin_for_cla    = 1'b0;
          reg_rd_data    = alu_add_result;
          // $display("[ALU-ADD] x%0d = x%0d + x%0d", inst_rd, inst_rs1, inst_rs2);
        end

        else if (inst_sub) begin
          alu_op2_in     = rs2_data;
          alu_op2_invert = 1'b1; // A + (~B + 1) = A - B
          cin_for_cla    = 1'b1;
          reg_rd_data    = alu_add_result;
          // $display("[ALU-SUB] x%0d = x%0d - x%0d", inst_rd, inst_rs1, inst_rs2);
        end

        else if (inst_sll) begin
          reg_rd_data = rs1_data << rs2_data[4:0];
        end

        else if (inst_slt) begin
          reg_rd_data = ($signed(rs1_data) < $signed(rs2_data)) ? 32'd1 : 32'd0;
        end

        else if (inst_sltu) begin
          reg_rd_data = (rs1_data < rs2_data) ? 32'd1 : 32'd0;
        end
          ...
          reg_rd_data = $signed(rs1_data) >>> rs2_data[4:0];
        end
          ...
          reg_rd_data = rs1_data * rs2_data;
          // $display("[MUL] x%0d = x%0d * x%0d", inst_rd, inst_rs1, inst_rs2);
        end

        else if (inst_div) begin
          reg_rd_data = div_quotient;
          // $display("[DIV ] x%0d = x%0d / x%0d = %0d", inst_rd, inst_rs1, inst_rs2, div_quotient);
        end

        else if (inst_divu) begin
          reg_rd_data = div_quotient;
          // $display("[DIVU] x%0d = x%0d / x%0d = %0d (unsigned)", inst_rd, inst_rs1, inst_rs2, div_quotient);
        end

        else if (inst_rem) begin
          reg_rd_data = div_remainder;
          // $display("[REM ] x%0d = x%0d %% x%0d = %0d", inst_rd, inst_rs1, inst_rs2, div_remainder);
        end

        else if (inst_remu) begin
          reg_rd_data = div_remainder;
          // $display("[REMU] x%0d = x%0d %% x%0d = %0d (unsigned)", inst_rd, inst_rs1, inst_rs2, div_remainder);
        end
