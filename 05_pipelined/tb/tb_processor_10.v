...
module tb_processor_10;
  ...
  task run_checks;
    begin
      $display({`BLUE, "---------- CHECKING REGISTERS (CASE10) ----------", `RESET});

      // x0 = 0
      check_reg(0, 32'h00000000);

      // Expected theo case10.asm
      // addi x1, x0, 123  => 123 = 0x7B
      // lw   x2, 0(x0)    => 123
      // add  x3, x2, x2   => 246 = 0xF6
      // add  x4, x3, x2   => 369 = 0x171
      check_reg(1, 32'h0000007b); // 123
      check_reg(2, 32'h0000007b); // 123
      check_reg(3, 32'h000000f6); // 246
      check_reg(4, 32'h00000171); // 369
  ...
  initial begin
    ...
    $display({`YELLOW, "  tb_processor_10: CASE10 - Mem→ALU Hazard", `RESET});
    $display({`YELLOW, "============================================", `RESET});
    ...
    if (err == 0)
      $display({`GREEN, "==== CASE10 PASS ====", `RESET});
    else
      $display({`RED, "==== CASE10 FAIL, err = %0d ====", `RESET}, err);
  ...
  // VCD
  initial begin
    $dumpfile("pipelined_processor_10.vcd");
    $dumpvars(0, tb_processor_10);
  end
