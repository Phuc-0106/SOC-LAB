...
module tb_processor_9;
  ...
  task run_checks;
    begin
      $display({`BLUE, "---------- CHECKING REGISTERS (CASE9) ----------", `RESET});

      // x0
      check_reg(0, 32'h00000000);

      // Expected theo case 9.asm
      // addi x1, x0, 1
      // addi x2, x0, 2
      // add  x3, x1, x2     # = 3
      // add  x4, x3, x2     # = 5
      ...
      check_reg(5, 32'h00000008);
      check_reg(6, 32'h0000000d);
      check_reg(7, 32'h00000015);
  ...
  initial begin
    ...
    $display({`YELLOW, "  tb_processor_9: CASE9 - MX/WX Bypass Test", `RESET});
    $display({`YELLOW, "============================================", `RESET});
    ...
    if (err == 0)
      $display({`GREEN, "==== CASE9 PASS ====", `RESET});
    else
      $display({`RED, "==== CASE9 FAIL, err = %0d ====", `RESET}, err);
  ...
  // VCD
  initial begin
    $dumpfile("pipelined_processor_9.vcd");
    $dumpvars(0, tb_processor_9);
  end
