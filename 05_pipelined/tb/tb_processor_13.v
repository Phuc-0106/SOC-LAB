...
module tb_processor_13;
  ...
  task run_checks;
    begin
      $display({`BLUE, "---------- CHECKING REGISTERS (CASE13 – WD bypass) ----------", `RESET});
      ...
      // addi x2, x0, 7    -> 7
      // add  x3, x1, x2   -> 12
      // add  x4, x3, x0   -> 12 (phải đọc qua WD bypass)
      check_reg(1, 32'h00000005); // x1 = 5
      check_reg(2, 32'h00000007); // x2 = 7
      check_reg(3, 32'h0000000c); // x3 = 12
      check_reg(4, 32'h0000000c); // x4 = 12 (WD bypass)
  ...
  initial begin
    ...
    $display({`YELLOW, "  tb_processor_13: CASE13 - WD Bypass Test", `RESET});
    $display({`YELLOW, "============================================", `RESET});
    ...
    if (err == 0)
      $display({`GREEN, "==== CASE13 PASS ====", `RESET});
    else
      $display({`RED, "==== CASE13 FAIL, err = %0d ====", `RESET}, err);
  ...
  // VCD
  initial begin
    $dumpfile("pipelined_processor_13.vcd");
    $dumpvars(0, tb_processor_13);
  end
