...
module tb_processor_16;
  ...
  parameter TIMEOUT_CYCLES = 400;  // cần rộng cho DIV + stall hazard
  ...
  task run_checks;
    begin
      $display({`BLUE, "---------- CHECKING REGISTERS (CASE16) ----------", `RESET});

      // Chương trình assembly:
      //   addi x1, x0, 100
      //   addi x2, x0, 3
      //   div  x3, x1, x2     # x3 = 33
      //   div  x4, x3, x2     # x4 = 11, phải stall tới khi x3 có kết quả
      //
      // Kỳ vọng:
      //   x0 = 0
      //   x1 = 100
      ...
      check_reg(1, 32'd100);      // x1 = 100
      check_reg(2, 32'd3);        // x2 = 3
      check_reg(3, 32'd33);       // x3 = 33
      check_reg(4, 32'd11);       // x4 = 11
  ...
  initial begin
    ...
    $display({`YELLOW, "  tb_processor_16: CASE16 - DIV Hazard Chain", `RESET});
    $display({`YELLOW, "============================================", `RESET});
    ...
    if (err == 0)
      $display({`GREEN, "==== CASE16 PASS ====", `RESET});
    else
      $display({`RED, "==== CASE16 FAIL, err = %0d ====", `RESET}, err);
  ...
  // VCD
  initial begin
    $dumpfile("pipelined_processor_16.vcd");
    $dumpvars(0, tb_processor_16);
  end
